...
module.exports = async ({ beatmap_id = undefined, beatmapset_id = undefined, creator = undefined, limit = 1, mode, command }) => {
    try {
        let beatmap = []
        let { modenum, a_mode } = get_mode_detail({ mode: mode })
        let param = command && command == 'without' ? { b: beatmap_id, s: beatmapset_id, u: creator, a: 1, limit: limit } : { b: beatmap_id, s: beatmapset_id, u: creator, m: modenum, a: 1, limit: limit };
        let beatmaps = await BanchoAPI({ ver: 1, endpoint: 'get_beatmaps', param: param })
        for (let i = 0; i < beatmaps.length; i++) {
            beatmap[i] = new Beatmap({
                beatmap_id: beatmaps[i].beatmap_id, title: beatmaps[i].title, creator: beatmaps[i].creator,
