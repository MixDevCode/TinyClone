        ...
        if (check_type == 'bancho') {
            if (ver == 1) {
                ...
                for (let i = 0; i < best.length; i++) {
                    ...
                    top[i] = new Score({
                        beatmap_id: best[i].beatmap_id, score: best[i].score, combo: best[i].maxcombo,
                ...
                if (!no_bm) {
                    ...
                    for (let i = 0; i < top.length; i++) {
                        let data = new Beatmap({
                            title: beatmaps[i][0].title, creator: beatmaps[i][0].creator,
            ...
            for (var i = 0; i < best.scores.length; i++) {
                ...
                top[i] = new Score({
                    beatmap_id: best.scores[i].beatmap.beatmap_id, score: Number(best.scores[i].score),
                ...
                let data = new Beatmap({
                    beatmapset_id: beatmap.beatmapset_id, title: title, artist: artist, diff: diff,
            ...
            let best = await RippleAPI({
                endpoint: `/v1/users/scores/${type}`, mode: mode, options:
