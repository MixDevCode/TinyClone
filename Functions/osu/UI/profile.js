...
module.exports = ({ mode, refresh, modeicon, embed_color, lang, supporter, modename, username, id, pp, global_rank,
    ...
    if (field1) embed.addFields({ name: localText.performance, value: field1, inline: true })
    if (field2) embed.addFields({ name: localText.rank, value: field2, inline: true })
    if (online) embed.setFooter({ text: online, iconURL: online_icon })
    //if (supporter) embed.setFooter({text: localText.supporter, iconURL: get_icon({type: "osu_supporter"})})
    return embed
}